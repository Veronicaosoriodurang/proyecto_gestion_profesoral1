...
```mermaid
flowchart LR
    N["Navegador o Postman"] -->|"HTTP puerto 8074"| A["API gestion profesoral<br/>C# ASP.NET Core + Dapper"]
    A -->|"SQL parametrizado"| B[("SQL Server puerto 11472<br/>base gestion_local")]
```
...
| `ProyectosDeAula/db_scripts/sqlserver/gestion_profesoral.ss.sql` | El **DDL**: 223 líneas, 19 `CREATE TABLE`, 19 claves foráneas |
...
|---|---|---|
| API gestión profesoral | **8074** | v1 |
| SQL Server | **11472** | v1 |
...
```yaml
  api-gestion:
    environment:
      ConnectionStrings__SqlServer: "Server=sqlserver,1433;Database=gestion_local;User Id=sa;Password=Aplicacionweb123!;TrustServerCertificate=True;"
```
...
flowchart LR
    ...
    CMP --> API["api-gestion<br/>ConnectionStrings__SqlServer<br/>sobreescribe appsettings.json"]
...
```yaml
    environment:
      ...
      ConnectionStrings__SqlServer: "Server=sqlserver,1433;Database=gestion_local;User Id=sa;Password=${MSSQL_SA_PASSWORD};TrustServerCertificate=True;"
```
...
| `api_gestion\Repositorios` | La interfaz y el SQL con Dapper | **Capa 3 — datos**: no conoce HTTP |
...
New-Item -ItemType File `
  api_gestion\ApiGestion.csproj, api_gestion\Dockerfile,
  api_gestion\Program.cs, api_gestion\appsettings.json,
  api_gestion\Modelos\Programa.cs,
  api_gestion\Peticiones\ProgramaCrear.cs,
  api_gestion\Peticiones\ProgramaReemplazo.cs,
  api_gestion\Peticiones\ProgramaActualizar.cs,
  api_gestion\Repositorios\IRepositorioPrograma.cs,
  api_gestion\Repositorios\RepositorioProgramaSqlServer.cs,
  api_gestion\Servicios\IServicioPrograma.cs,
  api_gestion\Servicios\ServicioPrograma.cs,
  api_gestion\Controllers\ProgramaController.cs,
  api_gestion\Excepciones\NoEncontradoExcepcion.cs,
  api_gestion\pruebas\PruebaCapas.csproj,
...
`db/gestion_profesoral.sql` = el DDL dado **+ las cuatro correcciones +
...
### Paso 5 — La API
C# / ASP.NET Core + Dapper, capas con interfaces, para `programa`: el modelo, las tres peticiones (crear / reemplazo / actualizar), la interfaz y el repositorio, la interfaz y el servicio, y el controlador con los siete endpoints. Más `Program.cs` con el ensamblador, `Dockerfile` y el proyecto `pruebas/` con un repositorio falso en memoria. **Unos 12 archivos**, no 45: esa es la ganancia de haber escogido una sola tabla.
```mermaid
flowchart TB
    ...
    R["RepositorioProgramaSqlServer<br/>SQL con Dapper"] -.->|implementa| IR
    R --> BD[("tabla programa")]
...
```powershell
dotnet build api_gestion
dotnet run --project api_gestion\pruebas
```
...
curl http://localhost:8074/                                  # diagnostico: version v1
curl http://localhost:8074/api/programa             # total: 218
curl "http://localhost:8074/api/programa?limite=3"  # exactamente 3
curl http://localhost:8074/api/programa/1A01        # una fila
curl -i http://localhost:8074/api/programa/9Z99     # 404 con mensaje claro
```
...
IA. Y `db/gestion_profesoral.sql` tampoco: ya está hecho y se copia tal cual.
...
3. Los errores NO nos frenan. Si te pego un error, lo diagnosticas y me
   ...
   juntos lo que salga.
4. La base de datos YA VIENE DADA en db/gestion_profesoral.sql + db/iid.sh —
...
8. Trabajo en Windows con VS Code (terminal integrada de PowerShell) y
   ...
   puerto 8074 y SQL Server el 11472.
...
DADA en db/gestion_profesoral.sql + db/iid.sh — úsalos tal cual para montar
...
5. Todo en español, C# sobre ASP.NET Core (.NET 10) con Dapper y el SQL a
   la vista. API en el puerto 8074, SQL Server en el 11472.
...
|---|---|---|
| La estructura de carpetas · `9_checklist.md` · la `GUIA_IA1.md` · los documentos conceptuales · `.gitignore` | `1_constitution.md` (stack y convenciones) · `0_mapa_versiones.md` (las 4 versiones del módulo) · `docker-compose.yml` (nombres y puertos) | `2_spec` … `8_tasks` de la v1 · `db/gestion_profesoral.sql` · toda la API |
