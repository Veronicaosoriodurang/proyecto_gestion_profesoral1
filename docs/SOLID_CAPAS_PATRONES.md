...
│ Repositorios/RepositorioProgramaSqlServer.cs        │
│ El SQL a mano (Dapper): traduce filas ↔ objetos     │
│ Programa. NO conoce HTTP. NO decide negocio.        │
└────────────────┬────────────────────────────────────┘
                 │  ④ SELECT … FROM programa WHERE codigo = @codigo
                 ▼
          ┌───────────────┐
          │ BASE DE DATOS │  SQL Server — gestion_local
          └───────┬───────┘
                  │
...
|---|---|---|---|
| **Controller** | HTTP: rutas, códigos de estado, JSON | SQL y reglas de negocio | `Controllers/ProgramaController.cs` |
| **Servicio** | Las reglas del negocio (¿existe? ¿se puede?) | Saber de HTTP o del motor de BD | `Servicios/ServicioPrograma.cs` |
| **Repositorio** | El SQL y el mapeo fila ↔ objeto | Saber de HTTP o decidir negocio | `Repositorios/RepositorioProgramaSqlServer.cs` |
...
Pregunta legítima: la carpeta de modelos (`Modelos/Programa.cs`) existe en el
...
AGREGAR clases (`RepositorioProgramaPostgres : IRepositorioPrograma`) y
...
public class RepositorioProgramaPostgres : IRepositorioPrograma { /* … */ }
...
`RepositorioProgramaSqlServer` en las pruebas y el servicio ni se entera.

```csharp
// El repositorio FALSO de las pruebas (criterio 6): sin BD, misma interfaz
public class RepositorioFalsoEnMemoria : IRepositorioPrograma
{
    private readonly Dictionary<string, Programa> _datos = new();

    public Task<Programa?> ObtenerPorCodigoAsync(string codigo)
        => Task.FromResult(_datos.GetValueOrDefault(codigo));
    // ...los otros 4 métodos...
}

// y el servicio NI SE ENTERA:
var servicio = new ServicioPrograma(new RepositorioFalsoEnMemoria());
```

### I — Segregación de interfaces
Interfaces pequeñas y específicas: `IRepositorioPrograma` tiene SOLO los 5
métodos de datos de programa — no un "IRepositorioDeTodo" que obligue a
implementar métodos que no se usan.

```csharp
// ✅ La interfaz de la v1: SOLO los 5 métodos de datos de programa
public interface IRepositorioPrograma
{
    Task<List<Programa>> ObtenerTodosAsync(int limite);
    Task<Programa?> ObtenerPorCodigoAsync(string codigo);
...
```csharp
public ServicioPrograma(IRepositorioPrograma repositorio)  // ← interfaz, no clase
```
...
```csharp
// El contrato (Repositorios/IRepositorioPrograma.cs):
Task<Programa?> ObtenerPorCodigoAsync(string codigo);

// ServicioPrograma lo usa SIN saber si detrás hay SQL Server, otro motor
...
public ServicioPrograma(IRepositorioPrograma repositorio) { … }
...
builder.Services.AddScoped<IRepositorioPrograma>(_ => fabrica.CrearRepositorioPrograma());
...
(`RepositorioProgramaSqlServer`, el falso de las pruebas — y los
