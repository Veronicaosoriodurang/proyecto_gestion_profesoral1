...
Y en las capas: `RepositorioProgramaSqlServer` guarda su cadena de conexión
...
interfaz. `ServicioPrograma` funciona igual con
`RepositorioProgramaSqlServer` (el real) que con `RepositorioFalsoEnMemoria`
(el de la prueba de capas) — porque ambos firman `: IRepositorioPrograma`.
...
(`IServicioPrograma`, `IRepositorioPrograma`) son abstracción pura: declaran
...
classDiagram
    ...
    class RepositorioProgramaSqlServer {
    ...
    RepositorioProgramaSqlServer ..|> IRepositorioPrograma : POLIMORFISMO
    ...
    note for RepositorioProgramaSqlServer "ENCAPSULAMIENTO: la conexión y el SQL no salen de aquí"
```
...
{
    Task<bool> CrearAsync(Programa datos);
}
...
{
    public Task<bool> CrearAsync(Programa datos)
...
classDiagram
    ...
    ServicioMal --|> RepositorioProgramaSqlServer : hereda del CONCRETO — quedó casado con SQL Server
...
sequenceDiagram
    ...
    participant C as ProgramaController
...
> modelos viven en `Modelos/`; en la v1 hay uno: `Programa`.
...
| En la v1 | `Programa` | `ProgramaCrear`, `ProgramaReemplazo`, `ProgramaActualizar` |
...
body `{"stock": 99}` muere en 422 contra `ProgramaReemplazo` y pasa
contra `ProgramaActualizar`. Cuando todos los verbos comparten forma, la
...
- **Propiedades con `required`**: no se puede construir un `Programa` sin
...
- **La petición declara y el framework valida**: las peticiones por verbo
  (`ProgramaCrear`, `ProgramaReemplazo`, `ProgramaActualizar`) llevan sus
...
ProgramaCrear / Reemplazo / Actualizar ← las PETICIONES: reglas por verbo (la frontera)
ProgramaController               ← objeto HTTP; compone un IServicioPrograma
...
RepositorioProgramaSqlServer     ← implementación concreta (encapsula ADO.NET y SQL)
...
4. En este repositorio: [SOLID_CAPAS_PATRONES.md](SOLID_CAPAS_PATRONES.md) y el código
   de `api_gestion/` (comentado línea a línea).
