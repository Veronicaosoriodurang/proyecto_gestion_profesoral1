...
| `[Route("api/programa")]` | `ProgramaController.cs` | "Todas mis rutas cuelgan de /api/programa" |
...
| `[FromBody] ProgramaCrear body` | parámetros del método | Captura el BODY: toma el JSON, lo vuelca en la PETICIÓN del verbo y lo VALIDA |

Cuando llega `GET /api/programa`, el enrutador compara el verbo y la ruta
de la petición contra los atributos de todos los controladores, encuentra
el método `Listar()` (que tiene `[HttpGet]` bajo `[Route("api/programa")]`)
...
1. ASP.NET routing     compara verbo+ruta contra los atributos
                       → elige ProgramaController.Crear()
2. Model binding       toma el JSON del body y lo vuelca en la petición ProgramaCrear
...
4. ProgramaController  construye la entidad Programa y llama al servicio
5. ServicioPrograma    delega al repositorio (por la interfaz)
6. RepositorioProgramaSqlServer
                       INSERT INTO programa (...) VALUES (@codigo, ...)  ← parametrizado
...
```mermaid
flowchart TD
    A["1. Kestrel recibe<br/>el texto HTTP"] --> B["2. ASP.NET Core encuentra<br/>POST /api/programa<br/>en la tabla de rutas"]
    B --> C{"3. ¿el body cumple las<br/>anotaciones de ProgramaCrear?<br/>(Required, Range...)"}
...
2. ProgramaController try { … } → llama al servicio
3. ServicioPrograma   pide al repositorio; si llega null → lanza
                      NoEncontradoExcepcion (el catch la vuelve 404)
4. Repositorio        SELECT ... WHERE codigo = @codigo
5. La fila vuelve convertida en objeto Programa y sale como JSON
...
Invoke-RestMethod "http://localhost:8074/api/programa"
Invoke-RestMethod "http://localhost:8074/api/programa/PR001"

# POST — crear
Invoke-RestMethod -Method Post -Uri "http://localhost:8074/api/programa" -ContentType "application/json" -Body '{"codigo":"PR009","nombre":"Webcam","stock":10,"valorunitario":350000}'

# PUT con body incompleto → error 422 (PUT exige TODO)
Invoke-RestMethod -Method Put -Uri "http://localhost:8074/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# PATCH con el MISMO body → 200 (PATCH es parcial)
Invoke-RestMethod -Method Patch -Uri "http://localhost:8074/api/programa/PR009" -ContentType "application/json" -Body '{"stock":25}'

# DELETE — limpiar
Invoke-RestMethod -Method Delete -Uri "http://localhost:8074/api/programa/PR009"
```
