# Plan técnico — Versión 1: `programa` (C# / ASP.NET Core)
...
Los archivos de la v1, todos dentro de `api_gestion/`:

```
api_gestion/
├── ApiGestion.csproj          los tres paquetes, y nada más
...
├── Modelos/
│   └── Programa.cs          la entidad: lo que viaja entre capas
├── Peticiones/                      la frontera de entrada — aquí nacen los 422
│   ├── ProgramaCrear.cs         los 4 campos, todos obligatorios
│   ├── ProgramaReemplazo.cs     los 3 campos del PUT, obligatorios
│   └── ProgramaActualizar.cs    los 3 del PATCH, todos opcionales
├── Servicios/                       CAPA 2 — negocio
│   ├── IServicioPrograma.cs     lo único que el controlador conoce
│   └── ServicioPrograma.cs
├── Repositorios/                    CAPA 3 — datos
│   ├── IRepositorioPrograma.cs  lo único que el servicio conoce
│   └── RepositorioProgramaSqlServer.cs   el SQL, parametrizado
├── Excepciones/
│   └── NoEncontradoExcepcion.cs     cómo el negocio dice "404" sin hablar de HTTP
├── Controllers/                     CAPA 1 — HTTP
│   └── ProgramaController.cs    los 7 endpoints
└── pruebas/
    ├── PruebaCapas.csproj
...
```csharp
builder.Services.AddScoped<IRepositorioPrograma,
                           RepositorioProgramaSqlServer>();
...
real**: solo conoce la interfaz `IRepositorioPrograma`.
...
{
    private readonly List<Programa> _filas = new();   // la "base de datos"

    public Task<Programa?> ObtenerPorId(string id) =>
...
```mermaid
flowchart LR
    S["ServicioPrograma"] --> I["IRepositorioPrograma<br/>interfaz"]
    R["RepositorioProgramaSqlServer<br/>en produccion"] -.->|implementa| I
...
```mermaid
flowchart LR
    P["Peticiones/<br/>ProgramaCrear"] --> C["Controller<br/>CAPA 1"]
    C -->|"construye la entidad"| M["Modelos/<br/>Programa"]
...
recibiera `ProgramaCrear`, cambiar la forma del cuerpo obligaría a
...
`api-gestion` (código montado, `dotnet watch`).
...
flowchart LR
    ...
    I -->|"service_completed_successfully"| A["api-gestion<br/>arranca con la base ya sembrada"]
```
...
|---|---|---|
| Puertos de `sqlserver` | `"11472:1433"` | 1433 es el puerto del motor adentro; 11472 el del host (Artículo 10) |
| Puertos de la API | `"8074:8074"` | La API escucha en **8074 también adentro**, no en 8080: así el `Dockerfile`, el `appsettings.json` y los contratos dicen el mismo número |
...
| Volúmenes de la API | `./api_gestion:/app` más `/app/bin` y `/app/obj` anónimos | El código montado es lo que permite `dotnet watch`. Los dos anónimos dejan los compilados **dentro** del contenedor: sin ellos se mezclan los binarios de Linux con los de Windows y la compilación falla con errores incomprensibles |
...
|---|---|
| **1** — Por versiones, la spec manda | Solo `programa`. Sin FK, sin JWT, sin front. Cierra con tag `v1` |
...
| **10** — Convenciones | Puertos 8074/11472, rutas `/`, `/swagger`, `/api/programa`, el sobre `{tabla, limite, total, datos}` y el catálogo de errores |
