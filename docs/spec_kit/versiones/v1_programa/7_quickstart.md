...
# 3. Crear y listar
curl -X POST http://localhost:8074/api/programa `
...
curl -X PUT http://localhost:8074/api/programa/9001 `
...
curl -i -X PATCH http://localhost:8074/api/programa/9001 `
  -H "Content-Type: application/json" -d '{"nombre":"X","tipo":"Y","fechaCreacion":"2005-01-15","numeroCohortes":"1","cantGraduados":"1","fechaActualizacion":"2026-01-01","ciudad":"Z","facultad":1}'
#    → 200: al PATCH le basta con lo enviado

# 5. El borrado es LÓGICO, y se comprueba
curl -X DELETE http://localhost:8074/api/programa/9001
#    → 200 filasAfectadas: 1
curl -i http://localhost:8074/api/programa
#    → 204 otra vez: el único programa desapareció del listado
curl -i -X DELETE http://localhost:8074/api/programa/9001
...
docker compose exec api-gestion dotnet run --project pruebas
...
persona, en **http://localhost:8075**:

1. entrar a **Programas**: se ve el listado (o el recuadro de «todavía no
