...
public class ServicioPrograma : IServicioPrograma
{
    ...
    {
        var programa = await _repositorio.ObtenerPorId(id);
        if (programa == null)
        {
            throw new NoEncontradoExcepcion($"No existe un programa con el código {id}.");
        }
    ...
    {
        await _repositorio.Crear(programa);
    }

    public async Task<int> Reemplazar(int id, Programa programa)
    {
        // El código identifica la fila y viene de la ruta, no del cuerpo
        programa.Id = id;

        var filasAfectadas = await _repositorio.Reemplazar(programa);
        if (filasAfectadas == 0)
        {
            throw new NoEncontradoExcepcion($"No existe un programa con el código {id}.");
        }
