...
public class ServicioExperiecia
{
    ...
    public ServicioExperiecia(HttpClient http)
    {
        _http = http;
    }

    public async Task<Resultado<List<Experiecia>>> Listar(int limite = 1000)
    {
        try
        {
            var r = await _http.GetAsync(
                $"/api/experiecia?limite={limite}");

            if (r.StatusCode == System.Net.HttpStatusCode.NoContent)
                return Resultado<List<Experiecia>>.Bien(new());

            if (!r.IsSuccessStatusCode)
                return Resultado<List<Experiecia>>.Mal(await Mensajes(r));

            var sobre = await r.Content.ReadFromJsonAsync<JsonElement>();

            var datos = sobre.GetProperty("datos")
                .Deserialize<List<Experiecia>>(_opciones) ?? new();

            return Resultado<List<Experiecia>>.Bien(datos);
        }
        catch (Exception e) when (
            e is HttpRequestException or TaskCanceledException)
        {
            return Resultado<List<Experiecia>>.Mal(NoDisponible);
        }
    }

    public async Task<Resultado<bool>> Crear(Experiecia entidad)
    {
        var cuerpo = new
        {
            nombreCargo = entidad.NombreCargo,
