...
public class ServicioRedDocente
{
    ...
    public ServicioRedDocente(HttpClient http)
    {
        _http = http;
    }

    public async Task<Resultado<List<RedDocente>>> Listar(int limite = 1000)
    {
        try
        {
            var r = await _http.GetAsync(
                $"/api/red-docente?limite={limite}");

            if (r.StatusCode == System.Net.HttpStatusCode.NoContent)
                return Resultado<List<RedDocente>>.Bien(new());

            if (!r.IsSuccessStatusCode)
                return Resultado<List<RedDocente>>.Mal(await Mensajes(r));

            var sobre = await r.Content.ReadFromJsonAsync<JsonElement>();

            var datos = sobre.GetProperty("datos")
                .Deserialize<List<RedDocente>>(_opciones) ?? new();

            return Resultado<List<RedDocente>>.Bien(datos);
        }
        catch (Exception e) when (
            e is HttpRequestException or TaskCanceledException)
        {
            return Resultado<List<RedDocente>>.Mal(NoDisponible);
        }
    }

    public async Task<Resultado<bool>> Crear(RedDocente entidad)
    {
        var cuerpo = new
        {
            red = entidad.Red,
            ...
            actDestacadas = entidad.ActDestacadas
        };
        ...
        RedDocente entidad)
    {
        ...
        return await Enviar(
            HttpMethod.Put,
            $"/api/red-docente/{redActual}/{docenteActual}",
            cuerpo);
    }
